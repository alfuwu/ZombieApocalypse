...
public abstract class Worm : ModNPC {
    ...
    internal virtual void OnTargetChange(int newTarget) { }
    ...
    public sealed override bool PreAI() {
        ...
        if (SegmentType == WormSegmentType.Head) {
            ...
            if (!NPC.HasValidTarget) {
                ...
                if (NPC.HasValidTarget) {
                    OnTargetChange(NPC.target);
                } else if (NPC.boss) { // If the NPC is a boss and it has no target, force it to fall to the underworld quickly
                    NPC.velocity.Y += 8f;
    ...
    public override void HitEffect(NPC.HitInfo hit) {
        if (Main.netMode != NetmodeID.Server && NPC.life <= 0) {
            int gore = Mod.TryFind($"{Name}_Gore", out ModGore g) ? g.Type : -1;
            if (gore > 0)
                Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, gore, 1f);
        }
    }
}
